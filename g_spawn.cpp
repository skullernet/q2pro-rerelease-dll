...
static void G_InitStatusbar()
{
    ...
    // picked up item
    sb.ifstat(STAT_PICKUP_ICON).xv(0).pic(STAT_PICKUP_ICON).xv(26).yb(-42).stat_string(STAT_PICKUP_STRING).yb(-50).endifstat();

    // selected item name
    sb.ifstat(STAT_SELECTED_ITEM_NAME).yb(-34).xv(319).stat_rstring(STAT_SELECTED_ITEM_NAME).yb(-58).endifstat();
...
void SP_worldspawn(edict_t *ent)
{
    ...
    for (size_t i = 0; i < q_countof(lightstyles); i++)
