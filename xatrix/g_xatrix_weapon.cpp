...
THINK(Trap_Gib_Think)(edict_t *ent) -> void {
    ...
    float degrees = (150.f * FRAME_TIME.seconds()) + ent->owner->delay;
    ...
    diff.normalize();
    ent->s.origin += diff * (15.0f * FRAME_TIME.seconds());
...
THINK(Trap_Think)(edict_t *ent) -> void {
    ...
    if (ent->s.frame > 4)
    {
        ...
        if (ent->s.frame == 8) {
            ...
            best = G_Spawn();
            best->count = ent->mass;
            best->x.scale = 1.f + ((ent->accel - 100.f) / 300.f) * 1.0f;
            SP_item_foodcube(best);
            best->s.origin = ent->s.origin;
            best->s.origin[2] += 24 * best->x.scale;
