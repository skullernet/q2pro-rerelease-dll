...
THINK(target_lightramp_think)(edict_t *self) -> void {
    ...
    style[0] = (char)('a' + self->movedir[0] + ((level.time - self->timestamp) / FRAME_TIME.seconds()).seconds() * self->movedir[2]);
    style[1] = 0;
...
void SP_target_lightramp(edict_t *self)
{
    ...
    self->movedir[2] = (self->movedir[1] - self->movedir[0]) / (self->speed / FRAME_TIME.seconds());
}
...
THINK(update_target_camera)(edict_t *self) -> void {
    ...
    {
        self->moveinfo.remaining_distance -= (self->moveinfo.move_speed * FRAME_TIME.seconds()) * 0.8f;
        ...
        if (self->moveinfo.remaining_distance <= 0) {
            ...
            if (self->enemy && (self->enemy->hackflags & HACKFLAG_TELEPORT_OUT))
                self->enemy->x.alpha = max(1.f / 255.f, frac);
...
THINK(target_camera_dummy_think)(edict_t *self) -> void {
    ...
    if (self->hackflags & HACKFLAG_TELEPORT_OUT)
    {
        ...
        self->x.alpha = max(1.f / 255.f, (self->timestamp.seconds() / self->pain_debounce_time.seconds()));
    }
