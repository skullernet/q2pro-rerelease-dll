...
{
    ent->velocity += ent->gravityVector * (ent->gravity * level.gravity * FRAME_TIME.seconds());
}
...
retry:
    ...
    for (part = ent; part; part = part->teamchain) {
        if (part->velocity[0] || part->velocity[1] || part->velocity[2] || part->avelocity[0] || part->avelocity[1] ||
            part->avelocity[2]) {
            // object is moving
            move = part->velocity * FRAME_TIME.seconds();
            amove = part->avelocity * FRAME_TIME.seconds();
...
void SV_Physics_Noclip(edict_t *ent)
{
    ...
    ent->s.angles += (ent->avelocity * FRAME_TIME.seconds());
    ent->s.origin += (ent->velocity * FRAME_TIME.seconds());
...
void SV_Physics_Toss(edict_t *ent)
{
    ...
    // move origin
    int num_tries = 5;
    float time_left = FRAME_TIME.seconds();
...
void SV_AddRotationalFriction(edict_t *ent)
{
    ...
    ent->s.angles += (ent->avelocity * FRAME_TIME.seconds());
    adjustment = FRAME_TIME.seconds() * sv_stopspeed->value * sv_friction; // PGM now a cvar
...
void SV_Physics_Step(edict_t *ent)
{
    ...
    if ((ent->flags & FL_FLY) && (ent->velocity[2] != 0) && !(ent->monsterinfo.aiflags & AI_ALTERNATE_FLY)) {
        ...
        newspeed = speed - (FRAME_TIME.seconds() * control * friction);
        if (newspeed < 0)
            newspeed = 0;
        newspeed /= speed;
    ...
    if ((ent->flags & FL_SWIM) && (ent->velocity[2] != 0) && !(ent->monsterinfo.aiflags & AI_ALTERNATE_FLY)) {
        speed = fabsf(ent->velocity[2]);
        control = speed < sv_stopspeed->value ? sv_stopspeed->value : speed;
        newspeed = speed - (FRAME_TIME.seconds() * control * sv_waterfriction * (float) ent->waterlevel);
        if (newspeed < 0)
            newspeed = 0;
        newspeed /= speed;
        ent->velocity[2] *= newspeed;
    ...
    if (ent->velocity[2] || ent->velocity[1] || ent->velocity[0]) {
        ...
        SV_FlyMove(ent, FRAME_TIME.seconds(), mask);
...
void G_RunEntity(edict_t *ent)
{
    ...
    if (ent->disintegrator_time) {
        ...
        ent->disintegrator_time = max(0_ms, ent->disintegrator_time - (15000_ms / TICK_RATE));

        if (ent->disintegrator_time)
            ent->x.alpha = max(1 / 255.f, 1.f - (ent->disintegrator_time.seconds() / 100.f));
        else
            ent->x.alpha = 1;
    }
#endif
