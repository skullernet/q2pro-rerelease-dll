...
{
    if (save_data_initialized)
        gi.error("attempted to create save_data_list at runtime");
...
struct save_type_deducer < T[N], typename std::enable_if_t < !std::is_same_v<T, char >>> {
    static constexpr save_field_t get_save_type(const char *name, size_t offset)
    {
        ...
        if (type.id <= ST_BOOL || type.id >= ST_DOUBLE)
            return { name, offset, { ST_FIXED_ARRAY, ST_INVALID, 0, []()
        {
            return save_type_deducer<std::remove_extent_t<T>>::get_save_type(nullptr, 0).type;
        } } };
...
struct save_type_deducer<std::array<T, N>> {
    static constexpr save_field_t get_save_type(const char *name, size_t offset)
    {
        ...
        if (type.id <= ST_BOOL || type.id >= ST_DOUBLE)
            return { name, offset, { ST_FIXED_ARRAY, ST_INVALID, N, []()
        {
            return save_type_deducer<std::remove_extent_t<T>>::get_save_type(nullptr, 0).type;
        } } };
...
static Json::Value parseJson(const char *filename)
{
    ...
    if (!json.isObject())
        gi.error("expected object at root");

    return json;
}

static void saveJson(const Json::Value &json, const char *filename)
{
...
void ReadGame(const char *filename)
{
    ...
    Json::Value json = parseJson(filename);
    ...
    if (!clients.isArray())
        gi.error("expected \"clients\" to be array");
    else if (clients.size() != game.maxclients)
        gi.error("mismatched client size");
...
void ReadLevel(const char *filename)
{
    ...
    // make sure to override these
    level.skyrotate = 0;
    level.skyautorotate = 0;
    ...
    if (!entities.isObject())
        gi.error("expected \"entities\" to be object");
