...
void SP_light(edict_t *self)
{
    ...
    if ((!self->targetname || (deathmatch->integer && !(self->spawnflags.has(SPAWNFLAG_LIGHT_ALLOW_IN_DM))))) { // [Sam-KEX]
        G_FreeEdict(self);
        return;
    }
...
TOUCH(barrel_touch)(edict_t *self, edict_t *other, const trace_t &tr, bool other_touching_self) -> void {
    ...
    M_walkmove(self, vectoyaw(v), 20 * ratio * FRAME_TIME.seconds());
}
...
THINK(misc_blackhole_think)(edict_t *self) -> void {
    ...
    {
        self->s.angles[0] += 50.0f * FRAME_TIME.seconds();
        self->s.angles[1] += 50.0f * FRAME_TIME.seconds();
    }
...
THINK(misc_hologram_think)(edict_t *ent) -> void {
    ent->s.angles[1] += 100 * FRAME_TIME.seconds();
    ent->nextthink = level.time + FRAME_TIME;
    ent->x.alpha = frandom(0.2f, 0.6f);
}
...
void SP_misc_player_mannequin(edict_t * self)
{
    ...
    self->mins *= self->x.scale;
    self->maxs *= self->x.scale;
