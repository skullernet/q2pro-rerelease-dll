...
void SV_CalcGunOffset(edict_t *ent)
{
    ...
    if (ent->client->pers.weapon &&
        ...
        for (i = 0; i < 3; i++) {
            ...
            if (d > 0)
                d = max(0.f, d - FRAME_TIME.milliseconds() * reduction_factor);
            else if (d < 0)
                d = min(0.f, d + FRAME_TIME.milliseconds() * reduction_factor);
        }
    }
    // ROGUE
    else {
...
void SV_CalcBlend(edict_t *ent)
{
    ...
    // add for contents
    vec3_t vieworg = ent->s.origin + ent->client->ps.viewoffset;
    ...
    if (ent->client->damage_alpha > 0)
        G_AddBlend(ent->client->damage_blend[0], ent->client->damage_blend[1], ent->client->damage_blend[2], ent->client->damage_alpha, ent->client->ps.screen_blend);
    ...
    ent->client->bonus_alpha -= FRAME_TIME.seconds();
    if (ent->client->bonus_alpha < 0)
...
void G_SetClientEffects(edict_t *ent)
{
    ...
    if (ent->client->invisible_time > level.time) {
        if (ent->client->invisibility_fade_time <= level.time)
            ...
            ent->x.alpha = std::clamp(x, 0.1f, 1.0f);
        }
    }
    // ROGUE
    //=======
    ...
    if (ent->disintegrator_time) {
        if (ent->disintegrator_time > 100_sec) {
            ...
            if (ent->disintegrator_time)
                ent->x.alpha = max(1 / 255.f, 1.f - (ent->disintegrator_time.seconds() / 100));
            else
                ent->x.alpha = 1;
        }
    }
...
void ClientEndServerFrame(edict_t *ent)
{
    ...
    if (level.intermissiontime || ent->client->awaiting_respawn) {
        if (ent->client->awaiting_respawn || (level.intermission_eou || level.is_n64 || (deathmatch->integer && level.intermissiontime))) {
            current_client->ps.screen_blend[3] = 0;//current_client->ps.damage_blend[3] = 0;
    ...
    if (xyspeed < 5) {
        ...
        if (xyspeed > 210)
            bobmove = FRAME_TIME.milliseconds() / 400.f;
        else if (xyspeed > 100)
            bobmove = FRAME_TIME.milliseconds() / 800.f;
        else
            bobmove = FRAME_TIME.milliseconds() / 1600.f;
    }
