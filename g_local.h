...
#define Com_PrintFmt(str, ...) \
...
#endif
    {
        ...
        game_import_t::dprintf("%s", print_buffer);
    }
...
#define Com_ErrorFmt(str, ...) \
    ...
    {
        G_FmtTo_(print_buffer, format_str, std::forward<Args>(args)...);
        game_import_t::error("%s", print_buffer);
    }
...
#define CLIP_BIT(ent) ((edict_t *)((uintptr_t)ent | 1))
    ...
    {
        edict_t *temp[MAX_EDICTS];
        int count = game_import_t::BoxEdicts(mins, maxs, temp, MAX_EDICTS, areatype);
        int j = 0;
    ...
    inline void SetAreaPortalState(int portalnum, bool open)
    {
...
struct spawnflags_t {
    ...
    constexpr bool has(const spawnflags_t &flags) const
    {
        return !!(value & flags.value);
    }
    // has all of the given flags ((a & b) == b)
    constexpr bool has_all(const spawnflags_t &flags) const
    {
...
extern gtime_t FRAME_TIME;

// view pitching times
inline gtime_t DAMAGE_TIME_SLACK()
{
    return (100_ms - FRAME_TIME);
}
...
public:
    ...
    constexpr const value_type pointer() const
    {
        return value;
    }
    constexpr const save_data_list_t *save_list() const
    {
        return list;
    }
    constexpr const char *name() const
    {
        return value ? list->name : "null";
    }
    ...
    constexpr bool operator==(value_type ptr_in) const
    {
        return value == ptr_in;
    }
    constexpr bool operator!=(value_type ptr_in) const
...
struct member_object_type<T1 T2::*> {
    using type = T1;
};
template<typename T>
...
public:
    ...
    inline edict_t *operator[](const difference_type &offset) const
    {
        return *(*this + offset);
    }
};
...
public:
    ...
    inline entity_iterator_t<TFilter> end() const
    {
        return end_index;
    }
};
...
inline void ThrowGibs(edict_t *self, int32_t damage, std::initializer_list<gib_def_t> gibs)
{
    for (auto &gib : gibs)
        for (size_t i = 0; i < gib.count; i++)
            ThrowGib(self, gib.gibname, damage, gib.type, gib.scale * (self->x.scale ? self->x.scale : 1));
}
