...
THINK(Move_Final)(edict_t *ent) -> void {
    ...
    ent->velocity = (ent->moveinfo.dest - ent->s.origin) * (1.f / FRAME_TIME.seconds());
...
THINK(Move_Begin)(edict_t *ent) -> void {
    ...
    if ((ent->moveinfo.speed * FRAME_TIME.seconds()) >= ent->moveinfo.remaining_distance)
    {
        Move_Final(ent);
        return;
    }
    ent->velocity = ent->moveinfo.dir * ent->moveinfo.speed;
    frames = floor((ent->moveinfo.remaining_distance / ent->moveinfo.speed) / FRAME_TIME.seconds());
    ent->moveinfo.remaining_distance -= frames * ent->moveinfo.speed * FRAME_TIME.seconds();
...
THINK(AngleMove_Final)(edict_t *ent) -> void {
    ...
    ent->avelocity = move * (1.0f / FRAME_TIME.seconds());
...
THINK(AngleMove_Begin)(edict_t *ent) -> void {
    ...
    if (traveltime < FRAME_TIME.seconds())
    {
        AngleMove_Final(ent);
        return;
    }

    frames = floor(traveltime / FRAME_TIME.seconds());
...
void SP_func_timer(edict_t *self)
{
    ...
    if (self->random >= self->wait) {
        self->random = self->wait - FRAME_TIME.seconds();
