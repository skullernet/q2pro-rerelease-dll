...
void pierce_trace(const vec3_t &start, const vec3_t &end, edict_t *ignore, pierce_args_t &pierce, contents_t mask)
{
    ...
    gi.dprintf("runaway pierce_trace\n");
}
...
THINK(Grenade4_Think)(edict_t *self) -> void {
    ...
    if (self->velocity)
    {
        ...
        self->s.angles.z = z + (FRAME_TIME.seconds() * 360 * speed_frac);
    }
...
void fire_rail(edict_t *self, const vec3_t &start, const vec3_t &aimdir, int damage, int kick)
{
    ...
    for (auto player : active_players()) {
        vec3_t org = player->s.origin + player->client->ps.viewoffset;

        if (binary_positional_search(org, start, args.tr.endpos, static_cast<game_import_t>(gi).inPHS, 3)) {
            gi.WriteByte(svc_temp_entity);
...
TOUCH(bfg_touch)(edict_t *self, edict_t *other, const trace_t &tr, bool other_touching_self) -> void {
    ...
    self->touch = nullptr;
    self->s.origin += self->velocity * (-1 * FRAME_TIME.seconds());
    self->velocity = {};
