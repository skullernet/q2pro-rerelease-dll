...
static bool SV_alternate_flystep(edict_t *ent, vec3_t move, bool relink, edict_t *current_bad)
{
    ...
    // FIXME
    if (std::isnan(final_dir[0]) || std::isnan(final_dir[1]) || std::isnan(final_dir[2]) || std::isnan(current_speed))
        return false;
    ...
    if (ent->enemy && (ent->monsterinfo.fly_buzzard || (ent->monsterinfo.aiflags & AI_MEDIC))) {
        ...
        d = vectoangles(d);
        ent->s.angles[PITCH] = LerpAngle(ent->s.angles[PITCH], -d[PITCH], FRAME_TIME.seconds() * 4.0f);
    } else
...
static bool SV_flystep(edict_t *ent, vec3_t move, bool relink, edict_t *current_bad)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (i == 0 && ent->enemy) {
            ...
            vec3_t &goal_position = ent->goalentity->s.origin;
...
void M_MoveToGoal(edict_t *ent, float dist)
{
    ...
    if (!(ent->monsterinfo.aiflags & AI_COMBAT_POINT) && ent->monsterinfo.attack_state < AS_MISSILE) {
        if (M_MoveToPath(ent, dist)) {
            ent->monsterinfo.path_blocked_counter = max(0_ms, ent->monsterinfo.path_blocked_counter - FRAME_TIME);
            return;
        }
    }
