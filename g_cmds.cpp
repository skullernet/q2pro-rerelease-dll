    ...
    if (i == GESTURE_POINT && has_a_target) {
        ...
        while ((targ = findradius(targ, ent->s.origin, 1024)) != nullptr) {
            ...
            if (aiming_at && other_notify_msg)
                gi.cprintf(targ, PRINT_TTS, other_notify_msg, ent->client->pers.netname, aiming_at->client->pers.netname);
            else if (other_notify_none_msg)
                gi.cprintf(targ, PRINT_TTS, other_notify_none_msg, ent->client->pers.netname);
        }

        if (aiming_at && other_notify_msg)
            gi.cprintf(ent, PRINT_TTS, other_notify_msg, ent->client->pers.netname, aiming_at->client->pers.netname);
        else if (other_notify_none_msg)
            gi.cprintf(ent, PRINT_TTS, other_notify_none_msg, ent->client->pers.netname);
    }
...
void Cmd_Say_f(edict_t *ent, bool arg0)
{
    ...
    if (sv_dedicated->integer)
        gi.cprintf(nullptr, PRINT_CHAT, "%s", text.c_str());
    ...
    for (uint32_t j = 1; j <= game.maxclients; j++) {
        ...
        if (!other->client)
            continue;
        gi.cprintf(other, PRINT_CHAT, "%s", text.c_str());
    }
}
#endif
...
void Cmd_PlayerList_f(edict_t *ent)
{
    ...
    if (text.length())
        gi.cprintf(ent, PRINT_HIGH, "%s", text.c_str());
}
...
void Cmd_Switchteam_f(edict_t* ent)
{
    ...
    if (g_teamplay_force_join->integer) {
        ...
        if (ent->client->resp.ctf_team != best_team) {
            ...
            Info_ValueForKey(ent->client->pers.userinfo, "skin", value, sizeof(value));
...
void ClientCommand(edict_t *ent)
{
    ...
    // anything that doesn't match a command will inform them
        ...
        gi.cprintf(ent, PRINT_HIGH, "invalid game command \"%s\"\n", gi.argv(0));
#endif
}
